...
/////////////////////////////////////////////////////////
void Modbus_Write_Send(u8 *data,u8 len)
{
    u8 i,j,temp[16];
    u16 crc;
    UART_485_TX;
  ...
  switch(MessAddr)
  {
    case 0x0500: //0,寄存器1：turb LED值，与365LED相关 寄存器2：cod LED值，与254LED相关
      ...
      switch(cmd)
      {
        ...
        case 0x10: for(i=0;i<len;i++)MODBUS.H0500[i] = temp[i+7]; //数据更新
                    Modbus_Write_Send(temp,len); break;//整体数据全部重写
      }
      break; 
    ...
    case 0x0700: //,寄存器1：硬件版本  寄存器2：软件版本
      ...
      switch(cmd)
      {
        ...
        case 0x10: for(i=0;i<len;i++)MODBUS.H0700[i] = temp[i+7];
                    Modbus_Write_Send(temp,len); break;
      }
      break; 
    ...
    case 0x0900: //,获取探头的序列号SN
      ...
      switch(cmd)
      {
        ...
        case 0x10: 
			memset(MODBUS.H0900,0,14);
			for(i=0;i<len;i++)MODBUS.H0900[i] = temp[i+7]; 
			Modbus_Write_Send(temp,len); break;
      }
      break; 
    ...
    case 0x1B00: //,获取/设置转刷上电自动转动flag
      ...
      switch(cmd)
      {
        ...
        case 0x10: for(i=0;i<len;i++)MODBUS.H1B00[i] = temp[i+7];
                    Modbus_Write_Send(temp,len); break;//全部数据更新
      }
    ...
    case 0x2500: //,开始测量(默认上电自动测量)
      ...
      switch(cmd)
      {
        case 0x03: Modbus_Read_Send(addr,cmd,len,MODBUS.H2500); 
		SampleIntervalTime=0; break;
      }
      break;
    ...
    case 0x3100: //,启动一次转刷清洗
      switch(cmd)
      {
        case 0x10: Modbus_Write_Send(temp,len); 
		MotorIntervalTime=0; break;
      }
      break;
    case 0x3200: //,获取/设置转刷转动间隔时间(min)
      ...
      switch(cmd)
      {
        case 0x03: Modbus_Read_Send(addr,cmd,len,MODBUS.H3200); 
				   MotorIntervalTimeBuf = (MODBUS.H3200[1]<<8) + MODBUS.H3200[0];break;
        case 0x10: for(i=0;i<len;i++)MODBUS.H3200[i] = temp[i+7];
                    Modbus_Write_Send(temp,len); 
					MotorIntervalTimeBuf = (MODBUS.H3200[1]<<8) + MODBUS.H3200[0];break;
      }
      break;
...
{
	FLASH_Read(0,MODBUS.H0500,sizeof(MODBUS));
	MotorIntervalTimeBuf = (MODBUS.H3200[1]<<8) + MODBUS.H3200[0];
	MotorIntervalTime = MotorIntervalTimeBuf;
	MotorState = (MODBUS.H1B00[1]<<8) + MODBUS.H1B00[0];
}
